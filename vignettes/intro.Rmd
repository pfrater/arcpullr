---
title: "Introduction to `arcgis.rest`"
...
The `arcgis.rest` package provides functions for pulling spatial data from 
...
However, the geometry object must be formatted to meet these query syntax 
specifications. This can be accomplished with the `format_*_coords` family of 
functions. In addition, you can also quickly create simple `sf` objects using
...
mke_waters <- get_layer_by_poly(url = streams_url, mke_format_poly)
```
